...
enum Opcode {
  ...
  CredentialsOpcode = 0x04, //unused by v3
  OptionsOpcode = 0x05,
...
fn opcode(val: u8) -> Opcode {
  match val {
    ...
    0x0C => EventOpcode,
    0x0D => BatchOpcode,
    0x0E => AuthChallengeOpcode,
    0x0F => AuthResponseOpcode,
    0x10 => AuthSuccessOpcode,
...
fn read_cql_string<T: io::Reader>(vec: &mut T) -> String {
  "test".to_string()
}

fn read_error_response(vec: Vec<u8>) -> Body {
  let mut reader = io::MemReader::new(vec);
  let code = reader.read_be_u32().unwrap();
  let len = reader.read_be_u16().unwrap();
  let string_bytes = reader.read_exact(len as uint).unwrap();
  let res = String::from_utf8(string_bytes);
  match res {
    result::Ok(string) => ErrorResponse(code, string),
    result::Err(_) => ErrorResponse(code, "couldn't parse".to_string())
  }
}

// impl<T: io::Reader> CqlReader for T {
fn read_cql_response<T: io::Reader>(buf: &mut T) -> Msg {
  let version = buf.read_u8().unwrap();
  let flags = buf.read_u8().unwrap();
  let stream = buf.read_i8().unwrap();
  let opcode = opcode(buf.read_u8().unwrap());
  let length = buf.read_be_u32().unwrap();

  println!("supposed length of body is {}", length);
  let body_data = buf.read_exact(length as uint).unwrap();
  // let body_data = buf.read_to_end().unwrap();
  // println!("body as string is {}", String::from_utf8(body_data).unwrap());

  let body = match opcode {
    AuthenticateOpcode => AuthenticateResponse(read_cql_string(buf)),
    ErrorOpcode => read_error_response(body_data),
    _ => EmptyResponse
  };
...
impl Client {
  pub fn query(&self, query: String, consistency: Consistency) -> Msg {
    let body = QueryRequest(query, consistency);
    let query_message = Msg {
      version: CQL_VERSION,
      flags: 0x00,
      stream: 0x01,
      opcode: QueryOpcode,
    ...
    query_message.serialize(&self.buf);
    self.buf.flush();

    query_message
  }

  pub fn buf(&self) -> io::BufferedStream<io::TcpStream> {
  }
}


pub fn connect(ip: String, port: u16) -> result::Result<Client, String> {

  let res = io::TcpStream::connect(ip.as_slice(), port);

  match res {
    result::Ok(stream) => {
      let client = Client { stream: res }
      let startup_msg = startup_request();
      let mut buf = io::BufferedStream::new(stream);
      startup_msg.serialize(&mut buf);
      let flushed = buf.flush();

      let msg = read_cql_response(&mut buf);
      match msg.opcode {
        ReadyOpcode => {
        ...
        AuthenticateOpcode => {
          println!("Auth required - sending credentials");
          result::Ok(CqlClient { buf: buf })
        },
        _ => {
          println!("Bad response - opcode was {}", msg.opcode);
          result::Err("Invalid response after startup".to_string())
        }
      }
    },
    result::Err(_) => {
      result::Err("Could not connect to IP".to_string())
    }
  }
}
